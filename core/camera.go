...
func NewCamera(pos, front mgl64.Vec3, fov, ratio float64) *Camera {
	...
	p.proj = mgl64.Perspective(mgl64.DegToRad(fov), ratio, 0.01, 1000)
	p.Update()
...
func (p *Camera) SetFov(fov float64) {
	p.fov = fov
	p.proj = mgl64.Perspective(mgl64.DegToRad(fov), p.ratio, 0.01, 1000)
}
